...
def generateCppFuncName(libname,namespace,classname,function):
	...
	if function==classname:#Constructor
		function=function+'C'
	elif function=='~'+classname:#Destructor
		function=classname+'D'
	elif function.strip().startswith("operator"):#oprand
		function=classname
		operator=True;
	else:
		pass
	...
	for func in result:
		...
		if operator and (not realfunc.startswith(classname+"::operator")):
			continue
...
def getSheerArgument(arguments):
	argList=arguments.split(',')
	argList=[x.split(' ')[-1].strip().strip('&').strip('*') for x in argList]
	for arg in argList:
		if not arg:
			print "member function argument set error,please check"
			sys.exit(2)
	return '' if not argList else ','+','.join(argList)
...
'''%option["headfile"]
	...
	argumentraw='' if not arguments else ','+arguments
	argumentarg=getSheerArgument(arguments)
...
'''%(option["function"],vpointValue,classname,argumentraw,option["libname"],namemangling,returnvalue,argumentarg,resultvalue)
